...
<div class="row no-gutters">
    @foreach($cats->courses->where('status', '1')->where('sort', 'instructor')->sortByDesc('id') as $c)
    @if($c->status == 1)
                    ...
                    <div class="view-block">
                        ...
                        <div class="video-device">
                            @if($c['preview_image'] !== NULL && $c['preview_image'] !== '')
                                <img src="{{ asset('images/course/'.$c['preview_image']) }}" class="bg_img img-fluid" alt="Background">
                            @else
                                <img src="{{ Avatar::create($c->title)->toBase64() }}" class="bg_img img-fluid" alt="Background">
                            @endif
                            @if($c->video !="" || $c->url !="")
                        ...
                        @endif
                        <div class="view-dtl" style=display:none;>
                            <div class="view-heading btm-10"><a href="{{ route('Instructor.course.show',['id' => $c->id, 'slug' => $c->slug ]) }}">{{ str_limit($c['title'], $limit = 35, $end = '...') }}</a></div>
...
@if($count >= 4)
    <div class="view-button txt-rgt">
        <a href="{{ route('category.page',['id' => $cats->id, 'category' => $cats->title]) }}" class="btn btn-secondary" title="View More">{{ __('frontstaticword.ViewMore') }}</a>
    </div>
@endif

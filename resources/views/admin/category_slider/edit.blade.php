	...
	<div class="row">
        <div class="col-md-12">
        	<div class="box box-primary">
				  ...
				  <div class="box-header with-border">
					<h3 class="box-title">حاضنه الأعمال التانيه</h3>
				 </div>
				  <div class="panel-body">
					<form action="{{ action('CategorySliderController@upd') }}" method="POST" enctype="multipart/form-data">
					  {{ csrf_field() }}
					  {{ method_field('PUT') }}
					  ...
					  <div class="row">
						<div class="col-md-6">
							...
							@foreach ($category as $cat)
							  @if($cat->status == 1)
							  <option value="{{ $cat->id }}" {{in_array($cat->id, $slider['category_id'] ?: []) ? "selected": ""}}>{{ $cat->title }}
							  </option>
							  @endif
